...
async function translateTextWithOpenAI(messages) {
...
function getPrompt(from, to, text) {
    return `${text}`;
}

function fixPunctuation(original, translated) {
    const punctuation = [".", "!", "?", ",", ":", ";", " "];

    for (const p of punctuation) {
        if (original.endsWith(p) && !translated.endsWith(p)) {
            translated += p;
        }
    }

    for (const p of punctuation) {
        if (translated.endsWith(p) && !original.endsWith(p)) {
            translated = translated.slice(0, -1);
        }
    }

    for (const p of punctuation) {
        if (original.startsWith(p) && !translated.startsWith(p)) {
            translated = p + translated;
        }
    }

    for (const p of punctuation) {
        if (translated.startsWith(p) && !original.startsWith(p)) {
